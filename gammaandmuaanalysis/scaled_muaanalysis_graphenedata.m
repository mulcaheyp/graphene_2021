clear all;clc
%% Initialize
% load annotations in the record of interest
load('')
...
% select channels with high quality electrophysiological recording
goodchannels = annotations.goodchans;
...
% MUA, and extract the phase locking behavior on all functional Gr channels
for n = 1:size(seizuretimes_in_indices,1)
    ...
    % behavior
    for g = 1:length(goodchannels)
        chan = seizure_data(:,goodchannels(g));
        % filter
        ...
        item2null{n,g} = lowphase(randi(length(lowphase),length(locs),1));
...
analysis_obj.muaamp = item2;
analysis_obj.nulls = item2null;
analysis_obj.Note = 'Rows are seizures. Columns are channels. Col 1 of each cell is time of MUA, Col2 is Phase locking 4-30 Hz.';

MUA_Analyses.analysis = analysis_obj;
