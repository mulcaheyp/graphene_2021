...
% import annotations for a given experimental record
load('')
...
% find and filter channels with high quality electrophysiological recording
goodchannels = annotations.goodchans;
for i = 1:length(goodchannels)
    chanchan = goodchannels(i);
...
for i = 1:size(seizuretimes_in_indices,1)
    ...
    for n = 1:length(goodchannels)
        % on each functional channel, detect the peak high gamma amplitudes
        ...
        lowphasedistribution{i,n} = lowphase(randi(length(lowphase),length(locs),1));
        clear pks locs
    end
...
analysis_obj.nulldistributions = lowphasedistribution;
...
HG_Analyses.analysis3 = analysis_obj;
